...
#获取参数
dependencyjars=`find "${basepath}/libs" -name '*.jar'|xargs echo | tr ' ' ','`
...
runparams="--driver-memory ${driver_memory} --driver-cores ${driver_cores} --num-executors ${num_executors} --executor-cores ${executor_cores} --executor-memory ${executor_memory}"
